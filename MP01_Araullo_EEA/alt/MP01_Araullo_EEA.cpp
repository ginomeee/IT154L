...
void table_checker_append(process_set process_setessing[],int process_setess);
void gantt_checker_append(process_set process_setessing[],int process_setess);
...
int main ()
{
	...
	int arrival_time[process_setess];
	int burst_time[process_setess];
	...
	for(int i = 0; i<sizeof(arrival_time)/sizeof(arrival_time[0]);i++){
	...
	for(int i = 0; i<sizeof(burst_time)/sizeof(burst_time[0]);i++){
...
void FirstCFS(int arrival_time[],int burst_time[],int process_setess, char choice){
	...
	if(choice == 'Y'){
		...
		gantt_checker_append(process_setessing, process_setess);
		table_checker_append(process_setessing, process_setess);
	}
}
...
////////////////////////SET GANTT TO WRITE FILE
void gantt_checker_append(process_set process_setessing[],int process_setess){
	//append
	int length = process_setessing[process_setess-1].end*3;
	int i, j;
 	ofstream ofs;
 	ofs.open("MP01 Checker.txt", ofstream::app);
 	ofs<<endl<<endl;
 	ofs<<" ";
    for(i=0; i<process_setess; i++) {
    ...
    for(i=0; i<process_setess; i++) {
        ...
        if(process_setessing[i].turnA > 9) ofs<<"\b"; // backspace : remove 1 space
